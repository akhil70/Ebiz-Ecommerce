...
async function run() {
  try {
    ...
    const adminAuth = await postJson('http://localhost:9000/api/auth/login', {
        username: 'admin',
        password: 'admin'
    });
    ...
    // 2. Login Seller
    console.log("Logging in as seller (shafeelunoffl@gmail.com)...");
    const sellerAuth = await postJson('http://localhost:9000/api/auth/login', {
        username: 'shafeelunoffl@gmail.com',
        password: '12345678'
    });
    const sellerToken = sellerAuth.access_token;
    ...
    for (const brand of brandsData) {
        const created = await postJson('http://localhost:9000/api/admin/brands', brand, adminToken);
        brandIds[brand.name] = created.id;
    ...
    for (const cat of categoriesData) {
        const created = await postJson('http://localhost:9000/api/admin/categories', cat, adminToken);
    ...
    for (const sub of subcategoriesData) {
        sub.categoryId = categoryIds[sub.categoryId];
        const created = await postJson('http://localhost:9000/api/admin/subcategories', sub, adminToken);
        subcategoryIds[sub.name] = created.id;
        console.log(`Created subcategory: ${sub.name} -> ID: ${created.id}`);
    }
    ...
    console.log(`\nAdding ${productsRaw.length} products to backend...`);
    for (let idx = 0; idx < productsRaw.length; idx++) {
        const p = productsRaw[idx];
        const bId = brandIds[p.brand];
        const cId = categoryIds[p.cat];
        const sId = subcategoryIds[p.sub];
        ...
        const created = await postJson('http://localhost:9000/api/v1/seller/products', productPayload, sellerToken);
        console.log(`[${idx+1}/50] Created product: ${p.name} -> ID: ${created.id}`);
        await sleep(100);
    }
